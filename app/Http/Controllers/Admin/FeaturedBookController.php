...
class FeaturedBookController extends Controller
{
    ...
    public function index()
    {
        ...
        $books = Book::all();
        return view('admin.books.featured', compact('books'));
    }
    ...
    public function store(Request $request)
    {
        ...
        Book::whereIn('id', $request->books)->update(['is_featured' => 1]);
    ...
    {
        $recommended_book->is_featured = 0;
        $recommended_book->save();

        return response()->json(['status' => 'success', 'message' => 'Book Removed from featured section Successful']);
    }
    ...
    protected function showData($request)
    {
        ...
        if (!empty($results)) {
            foreach ($results as $key => $row) {
                ...
                $id = $row->id;
                $del_link = route("admin.featured_book.destroy", ["featured_book" => $id]);
                $csrf = csrf_token();
